...
Exports the final connection state and recovery report after event processing.
Produces:
- connection_state.jsonl: per-connection state records
- recovery_report.json: summary metadata and pool accounting
"""
...
def export_connection_state(connections, pool, handler_stats, event_stats, output_dir):
    """Export connection states and recovery report.

    Args:
        connections: dict of conn_id -> Connection objects
        pool: ConnectionPool instance
        handler_stats: handler processing statistics
        event_stats: event processor statistics
        output_dir: path to write output files

    Returns:
        (state_path, report_path) tuple
    """
