...
Direction semantics:
- "local" events: PASSIVE_OPEN, ACTIVE_OPEN, CLOSE, TIMEOUT
- "remote" events: SYN_RECV, SYN_ACK_RECV, ACK_RECV, FIN_RECV
"""
...
# Events that are initiated locally vs received from remote
...
class ConnectionHandlers:
    ...
    def _handle_syn_recv(self, conn_id, source_addr, dest_addr, timestamp):
        ...
        # Reserve a pool slot for this connection
    ...
    def _handle_transition(self, conn_id, event_type, timestamp):
        """Generic transition handler — used for SYN_ACK_RECV and FIN_RECV."""
        result = {"conn_id": conn_id, "event_type": event_type,
                  "timestamp": timestamp, "handled": False, "action": None}
        ...
        # Use local direction as default for generic transitions
        success, new_state = self.state_machine.transition(
