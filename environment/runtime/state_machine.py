...
Implements an extended RFC-793 state model with half-close tracking.
...
class StateMachine:
    ...
    def transition(self, connection, event, timestamp, direction="local"):
        ...
        Args:
            ...
            direction: "local" or "remote" indicating initiator
        ...
        logger.debug("conn %s: %s -> %s (event=%s, dir=%s)",
                     connection.conn_id, old_state, new_state, event, direction)
